...
from ShoppingList.models import MyCars

def ProductDetails(request, IdNumber):
    if request.method=='POST':
        if 'Creat' in request.POST:#新建菜品
        ...
        formValue=ProductDetailForm({'ProductID':IdNumber[0:4],'ProductName':formValue.ProductName,
                                     'ProductPrice':formValue.ProductPrice,'ProductNumber':formValue.ProductNumber,
                                     'ProductOffer':formValue.ProductOffer
                                     })
        return render(request,'ProductDetail.html',{'form':formValue,'types':types})

def aaa(request):
    return render(request,'login/menuinfo.html')